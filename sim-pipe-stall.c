/* sim-pipe-stall.c - modified pipeline simulator implementation with branch stalls */

/* Based on SimpleScalar sim-pipe.c, modified to stall on branches rather than
 * flushing the pipeline. This provides a more accurate model of a processor
 * without branch prediction that stalls until branch resolution.
 */
 ...
 *  • Pipeline stalls on branches until resolution in the execute stage
...
struct pipeline_latch
{
  ...
  int is_branch; /* branch instruction bit */
};
...
/* branch stall statistics */
static counter_t sim_branch_stalls = 0;
...
init_latch(int stage)
{
  ...
  pipe_latch[stage].is_branch = FALSE;
}
...
{
  opt_reg_header(odb,
                 "sim-pipe-stall: This simulator implements a 5-stage pipeline simulator with branch stalls.\n"
                 "The simulator models the following assumptions:\n"
                 "  • Perfect instruction cache (no I-cache misses)\n"
                 "  • Perfect data cache (no D-cache misses)\n"
                 "  • No branch prediction - stall until branches are resolved\n"
                 "  • Split-phase register access for the writeback (WB) stage.\n"
                 "    This means that writes to registers occur in the first half\n"
                 "    of the clock cycle, and reads occur in the second half.\n");
...
void sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_branch_stalls",
                   "total number of branch stall cycles",
                   &sim_branch_stalls, 0, NULL);
...
/* Check if the pipeline contains a branch that's still resolving */
static int
has_unresolved_branch(void)
{
  int i;
  for (i = 0; i < PIPE_STAGES; i++)
  {
    if (pipe_latch[i].valid && pipe_latch[i].is_branch)
      return TRUE;
  }
  return FALSE;
}
...
void sim_main(void)
{
  ...
  int branch_stall = FALSE; /* Flag to indicate when pipeline is stalled due to a branch */
  ...
  while (TRUE)
  {
    ...
    /* Check if there's a branch in the pipeline that needs to stall subsequent stages */
    branch_stall = has_unresolved_branch();
    if (branch_stall)
      sim_branch_stalls++;
...
#include "machine.def"
      ...
      if (MD_OP_FLAGS(pipe_latch[E_STAGE].op) & F_CTRL)
      {
        ...
        /* A branch resolves in the execute stage */
        pipe_latch[M_STAGE].is_branch = FALSE; /* Branch is now resolved */

        /* If branch is taken, update PC for next fetch */
        if (target_PC != pipe_latch[E_STAGE].npc)
        {
          /* Update PC for the next fetch after stall */
    ...
    if (pipe_latch[D_STAGE].valid && !pipe_latch[D_STAGE].stall)
    {
      ...
      {
        pipe_latch[E_STAGE].is_branch = TRUE;
      }
    ...
    {
      /* If there's a branch in the pipeline, stall the fetch stage */
      if (branch_stall)
      {
        pipe_latch[F_STAGE].stall = TRUE;
        pipe_latch[D_STAGE].valid = FALSE;
      }
      else
      {
        /* No branches, proceed with fetch */
    ...
    else if (pipe_latch[F_STAGE].valid && pipe_latch[F_STAGE].stall && !branch_stall)
    {
      /* stall resolved, resume fetch */
      pipe_latch[F_STAGE].stall = FALSE;
      pipe_latch[D_STAGE].valid = FALSE;
    }
    else if (pipe_latch[F_STAGE].valid && pipe_latch[F_STAGE].stall && branch_stall)
    {
      /* continue stalling fetch */
      pipe_latch[D_STAGE].valid = FALSE;
    }
    else
    {
